...
<!--
현재 이 설계도를 AI 가 잘 인식하지 못하고 내가 원하는 output 을 주지 못하는 이유
현재 이설계도를 보고 AI의 OUTPUT 은
단일 ELF파일을 mmap해서 section header -> systemb/dynsym -> strtab만 타고 들어가서 심볼 뽑고 정렬, 필터, 출력 하는 기본 nm 의 기능은 돌아가는데, 아키이브 디테일, 리소스 정책, 우선순위 롤 은 구현계약으로 고정되지 않아서 거의 반영이 되지 않았다.
- “일단 ELF로 보이면 ELF로 가자”
- “ar은 복잡하니 나중에 / optional일 수 있다”
정책이 ‘테스트/검증 항목(DoD)’ 형태로 주어져야한다.
- 이 규칙은 per-symbol이 아니다. 반드시 ‘이름 그룹핑 → 우선순위 resolve → 그 결과를 출력’ 단계가 있어야 한다. 이런 문정아 존재하지 않았다.
현재 발생한문제는 구현계약이 부족해서 발생한 일이다.
1. **진입점/모듈 경계가 명확한가?** (어디서 ar을 처리해야 하지?)
2. **반드시 통과해야 하는 게이트가 명시됐나?** (ar이면 무조건 ar 루트를 타야 함)
3. **실패 정책이 테스트로 고정됐나?** (member 하나 오류면 continue? archive 전체 fail?)
4. **완성 기준(DoD)이 있나?** (이 테스트 못 통과하면 실패)

잘 전달하기 위한 방법
    - 설계도와 구현도 를 구분해야한다.
- 설계도(의도 / 정책 / 규칙)
    목적 : 무엇을 만들고, 어떤 원칙/정책으로 동작해야 하는가 를 정의
    독자 : 사람(라뷰어 / 동료 / 미래의 나 ) + AI(의도 이해)
    특징 :
        - 정책(프로그램 종료 기준, 스킵 기준, fatal 기준)
        - 규칙(심볼 타입 규칙, ar_name 규칙, 범위검사 원칙)
        - 우선순위(P0/P1)
        - 제약(허용 함수, 디렉토리 제한, 주석 금지)
    설계도는 "방향과 기준"이 핵심이다.

- 구현도(코드로 바로 옮길 수 있는 계약/인터페이스/테스트)
    목적 : AI가 빠트릴 수 없게, 코드 구조를 가ㅇ제로 만들기
    독자 : AI + 구현자(미래의 나)
    특징 :
        - 모듈 분해(어떤 파일/모듈이 어떤 책임)
        - 함수 계약(입력 / 출력 / 에러 / 리소스)
        - 상태머신(리소스 라이프사이클. cleanup 경로)
        - 테스트/DoD(이케이스 통과 못 하면 실패)
        - 규칙도("문장"이 아니라 "구현 가능한 형태")
    구현도는 "강제력"이 핵심
- total
    - 설계도는 "철학 / 정책 / 규칙"
    - 구현도는 "구조 / 계약 / 테스트"
